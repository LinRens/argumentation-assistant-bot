...
def start_message(message):
    ...
    key_joke = telebot.types.InlineKeyboardButton(text='Хочу анекдот!', callback_data='joke')
...
def callback_worker(call):
    ...
    if call.data == 'news':
        ...
        for topic in categories:
            key_topic = telebot.types.InlineKeyboardButton(text=str(topic), callback_data=topic)
            keyboard1.add(key_topic)
        key_random = telebot.types.InlineKeyboardButton(text='Не знаю, слишком сложно',
                                                        callback_data=random.choice(list(categories.keys())))
    ...
    if call.data in categories:
        topic = call.data
        bot.send_message(call.message.chat.id, 'Ты выбрал категорию ' + str(topic),  parse_mode="Markdown")
        bot.send_message(call.message.chat.id, random.choice(get_news(url, categories[topic])),  parse_mode="Markdown")
